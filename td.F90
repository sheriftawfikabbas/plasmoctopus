...
module td_oct_m
  ...
  type td_t
    ...
    !PlasmOctopus begin
    !Purpose: plasmonics
    logical              :: plasmonics_calculate
    logical              :: plasmonics_output_wfmod
    logical              :: td_current_through_plane
    integer              :: plasmonics_nomega
    FLOAT,       pointer :: plasmonics_omega(:) 
    FLOAT                :: plasmonics_sigma 
    !Purpose: electric enhancement factor
    logical              :: electric_enhancement_calculate_omega
    logical              :: electric_enhancement_calculate_td
    integer              :: electric_enhancement_nomega
    FLOAT,       pointer :: electric_enhancement_omega(:) 
    integer              :: electric_enhancement_ntd
    integer,       pointer :: electric_enhancement_td(:) 
    FLOAT                :: electric_enhancement_sigma
    !PlasmOctopus end
  end type td_t
...
contains
  ...
  subroutine td_init(td, sys)
    ...
    td%plasmonics_calculate = .false.
    td%plasmonics_output_wfmod = .false.
    
    !%Variable PlasmonicsSigma
    !%Type float
    !%Default 0.0
    !%Section Time-Dependent::Plasmonics
    !%Description
    !%End
    call parse_variable(sys%namespace,'PlasmonicsSigma', CNST(0.0), td%plasmonics_sigma, unit = units_inp%energy)
    !td%plasmonics_sigma = units_to_atomic(units_inp%energy,  td%plasmonics_sigma) ==> This will apply unit conversion twice!
    !write(*,'(a,F20.10)') 'PlasmOctopus###################plasmonics_sigma = ', td%plasmonics_sigma
    
    !%Variable PlasmonicsOmegas
    !%Type block
    !%Default no
    !%Section Time-Dependent::Plasmonics
    !%Description
    !% The energies at which the modes will be written
    !%End
    if(parse_block(sys%namespace, 'PlasmonicsOmegas', blk) .eq. 0) then
      message(1) = "<><><><><><> PlasmOctopus td_init_inc.F90 ------ inside plasmonics_calculate"
      call messages_info(1)    
      
      !%Variable PlasmonicsOutputWfmod
      !%Type logical
      !%Default no
      !%Section Time-Dependent::Plasmonics
      !%Description
      !%End
      call parse_variable(sys%namespace, 'PlasmonicsOutputWfmod', .false., td%plasmonics_output_wfmod)

      td%plasmonics_calculate = .true.
      td%plasmonics_nomega = parse_block_n(blk)
      SAFE_ALLOCATE(td%plasmonics_omega(1:td%plasmonics_nomega))
      row_loop1: do i = 1, td%plasmonics_nomega
        call parse_block_float(blk, i-1, 0, td%plasmonics_omega(i))
        td%plasmonics_omega(i) = units_to_atomic(units_inp%energy,  td%plasmonics_omega(i))
        write(*,'(a,F20.10)') 'PlasmOctopus###################plasmonics_omega = ', td%plasmonics_omega(i)
      end do row_loop1
    end if
    !call parse_block_end(blk)
    
    td%electric_enhancement_calculate_omega = .false.
    !%Variable ElectricEnhancementFactorSigma
    !%Type float
    !%Default 0.0
    !%Section Time-Dependent::Plasmonics
    !%Description
    !%End
    call parse_variable(sys%namespace,'ElectricEnhancementFactorSigma', CNST(0.0), td%electric_enhancement_sigma, unit = units_inp%length)
    
    !%Variable ElectricEnhancementFactorOmegas
    !%Type block
    !%Default no
    !%Section Time-Dependent::Plasmonics
    !%Description
    !% The energies at which the electric enhancement will be written
    !%End
    if(parse_block(sys%namespace,'ElectricEnhancementFactorOmegas', blk) .eq. 0) then
      td%electric_enhancement_calculate_omega = .true.
      td%electric_enhancement_nomega = parse_block_n(blk)
      SAFE_ALLOCATE(td%electric_enhancement_omega(1:td%electric_enhancement_nomega))
      row_loop2: do i = 1, td%electric_enhancement_nomega
        call parse_block_float(blk, i-1, 0, td%electric_enhancement_omega(i))
          td%electric_enhancement_omega(i) = units_to_atomic(units_inp%energy,  td%electric_enhancement_omega(i))
      end do row_loop2
    end if
    !call parse_block_end(blk)
    td%electric_enhancement_calculate_td = .false.

    !%Variable ElectricEnhancementFactorTD
    !%Type block
    !%Default no
    !%Section Time-Dependent::Plasmonics
    !%Description
    !% Can't remember
    !%End
    if(parse_block(sys%namespace,'ElectricEnhancementFactorTD', blk) .eq. 0) then
      td%electric_enhancement_calculate_td = .true.
      td%electric_enhancement_ntd = parse_block_n(blk)
      SAFE_ALLOCATE(td%electric_enhancement_td(1:td%electric_enhancement_ntd))
      row_loopEFE_TD: do i = 1, td%electric_enhancement_ntd
        call parse_block_integer(blk, i-1, 0, td%electric_enhancement_td(i))
      end do row_loopEFE_TD
    end if
    !call parse_block_end(blk)
    !PlasmOctopus end
  ...
  subroutine td_run(sys, fromScratch)
    ...
    !PlasmOctopus begin
    integer, allocatable 	:: ix(:)	!to hold the values
              !of the integer xyz coordinates
    FLOAT, allocatable :: xx(:)
    FLOAT, allocatable :: xxlength(:)
    integer 		:: ip, inst, inik, idir, iatom, idim, iomega, ip_prime, max_ip, iii, rankmin, ip_x, ispin
    integer     :: file_plasmonic_mode
    integer     :: file_electric_enhancement_mode
    integer     :: file_current
    integer     :: file_current_density
    FLOAT			:: sqr_wf,sqr_wf2, r, c, dmin, E_unit_conversion
    FLOAT			:: P_C !  The fine-structure constant
    CMPLX, allocatable :: zpsi_temp(:, :, :, :)
    FLOAT, allocatable :: dpsi_temp(:, :, :, :)
    !FLOAT, allocatable :: rho_region(:,:)         !< rho_region(gr%mesh%np_part, st%d%nspin)
    FLOAT, allocatable :: rho_0(:,:)         !the density at time = 0 (to get the density difference)

    !Electromagnetic Enhancement Factor stuff
    FLOAT,             allocatable :: electric_field_effective(:,:,:) 
    FLOAT,             allocatable :: electric_field_effective_td(:,:,:,:) 
    FLOAT,             allocatable :: total_potential_effective(:)
    FLOAT,             allocatable :: electric_enhancement_FFT(:,:,:,:)
    logical            :: electric_enhancement_td_found
    !Plasmonics
    !FLOAT,             allocatable :: density_FFT_integrand(:,:,:)
    FLOAT,             allocatable :: density_FFT(:,:,:)
    !FLOAT,             allocatable :: wfmod_FFT_integrand(:,:,:,:,:)
    FLOAT,             allocatable :: wfmod_FFT(:,:,:,:,:)
    CMPLX,             allocatable :: wfmod_0(:,:,:,:)
    character(len=256) :: plasmonics_filename
    character(len=256) :: electric_enhancement_filename
    character(len=256) :: current_density_filename
    character(len=256)     :: electric_enhancement_td_filename
    FLOAT            :: leftmaxz,rightminz
    !PlasmOctopus end
    PUSH_SUB(td_run)
    ...
    !PlasmOctopus begin
    c = CNST(299792458)*CNST(1e-5)/CNST(0.6582122)
    !Plasmonics
    if(td%plasmonics_calculate) then
      !SAFE_ALLOCATE(dpsi_temp(1:gr%mesh%np_part, 1:st%d%dim, st%st_start:st%st_end, st%d%kpt%start:st%d%kpt%end))
      SAFE_ALLOCATE(zpsi_temp(1:gr%mesh%np_part, 1:st%d%dim, st%st_start:st%st_end, st%d%kpt%start:st%d%kpt%end))
      SAFE_ALLOCATE(density_FFT(1:gr%fine%mesh%np_part, 1:td%plasmonics_nomega, 1:sys%st%d%nspin))
      SAFE_ALLOCATE(rho_0(1:gr%fine%mesh%np, 1:st%d%nspin))

      density_FFT = M_ZERO
       
      !SAFE_ALLOCATE(electric_field_effective(1:sys%gr%der%mesh%np, 1:sys%gr%mesh%sb%dim, 1:sys%st%d%nspin))
      !SAFE_ALLOCATE(electric_field_effective_td(1:sys%gr%mesh%np, 1:sys%gr%mesh%sb%dim, 1:td%electric_enhancement_ntd, 1:sys%st%d%nspin))
      !SAFE_ALLOCATE(total_potential_effective(1:sys%hm%psolver%der%mesh%np))
      !SAFE_ALLOCATE(electric_enhancement_FFT(1:sys%gr%mesh%np, 1:sys%gr%mesh%sb%dim, 1:td%electric_enhancement_nomega, 1:sys%st%d%nspin))
      !electric_field_effective = M_ZERO
      !total_potential_effective = M_ZERO
      !electric_enhancement_FFT = M_ZERO
    end if
    
    if(td%plasmonics_output_wfmod) then
      SAFE_ALLOCATE(wfmod_FFT(st%st_start:st%st_end, st%d%kpt%start:st%d%kpt%end, 1:sys%hm%psolver%der%mesh%np, 1:td%plasmonics_nomega, 1:sys%st%d%nspin))
      SAFE_ALLOCATE(wfmod_0(st%st_start:st%st_end, st%d%kpt%start:st%d%kpt%end, 1:sys%hm%psolver%der%mesh%np, 1:sys%st%d%nspin))
      wfmod_FFT = M_ZERO
      wfmod_0= M_ZERO
    end if
    ...
    !------------------------------------------------
    !Electromagnetic Enhancement Factor stuff
    if(td%electric_enhancement_calculate_td .or. td%electric_enhancement_calculate_omega) then
      SAFE_ALLOCATE(electric_field_effective(1:sys%gr%der%mesh%np_part, 1:sys%gr%mesh%sb%dim, 1:sys%st%d%nspin))
      SAFE_ALLOCATE(electric_field_effective_td(1:sys%gr%mesh%np_part, 1:sys%gr%mesh%sb%dim, 1:td%electric_enhancement_ntd, 1:sys%st%d%nspin))
      SAFE_ALLOCATE(total_potential_effective(1:sys%hm%psolver%der%mesh%np_part))
      SAFE_ALLOCATE(electric_enhancement_FFT(1:sys%gr%mesh%np_part, 1:sys%gr%mesh%sb%dim, 1:td%electric_enhancement_nomega, 1:sys%st%d%nspin))
      electric_field_effective = M_ZERO
      total_potential_effective = M_ZERO
      electric_enhancement_FFT = M_ZERO
    end if
    ...
    !PlasmOctopus begin
    !The charge density at t=0
    do ip = 1,gr%mesh%np
      do ispin=1,st%d%dim
        rho_0(ip,ispin)=st%rho(ip,ispin)
        
      end do
    end do
    if(td%plasmonics_output_wfmod) then
      do inik = st%d%kpt%start,st%d%kpt%end
        do inst = st%st_start,st%st_end
          call states_elec_get_state(st, sys%hm%psolver%der%mesh, inst, inik, wfmod_0(:, :, inst, inik))
        end do
      end do
    end if
    !PlasmOctopus end
    ...
    propagation: do iter = td%iter, td%max_iter
      ...
      !PlasmOctopus begin
      ! Calculate the induced electric field electric enhancement factor:
      ! To calculate: the electric enhancement factor EF_{EM} (Seth M. Morton et al., Chem. Rev. 2011, 111, 3962–3994).
      ! Using Eq. 4 in J. Chem. Phys. 134, 074701 (2011), and suppl materials of Nano Lett. 2012, 12, 1333.
      ! The electric field is just the gradient of the fourier transform of the effective potential. So easy.
      ! Applying the sine spectrum which is the default in Octopus in calculating the polarizability.
      ! Should also provide a "spectrum" for the enhancement factor at certain points in space.
      if(td%electric_enhancement_calculate_omega .or. td%electric_enhancement_calculate_td) then
        do ispin = 1, st%d%nspin
          !This line below is wrong: we should have the whole potential
          !total_potential_effective(1:sys%gr%mesh%np) = sys%hm%vhartree(1:sys%gr%mesh%np)
          
          total_potential_effective(:) = sys%hm%vhxc(:, ispin) + sys%hm%ep%vpsl(:)

          !write(message(1), '(a,i10.10,a,i10.10)') 'PlasmOctopus  >>>> ', ubound(total_potential_effective(:), DIM=1), ' ', gr%der%mesh%np_part
          !call messages_info(1)

          call dderivatives_grad(gr%der, total_potential_effective(:), electric_field_effective(:, :, ispin))
          
          electric_field_effective(:, :, ispin)=-1*electric_field_effective(:, :, ispin)
          
          !Now write the potential at (0,0,0), should be generalized later
          !xx = 0.0
          !ip_x=mesh_nearest_point(gr%mesh,xx,dmin,rankmin)
          !E_unit_conversion = units_from_atomic((units_inp%energy / units_inp%length),1.0)
          !write(message(1), '(a,i1,e20.12,a,e20.12,e20.12,e20.12,a)') 'PlasmOctopus EFE: Spin = ',ispin, E_unit_conversion, '(', electric_field_effective(ip_x, 1, ispin),electric_field_effective(ip_x, 2, ispin),electric_field_effective(ip_x, 3, ispin), ')'
          ! call messages_info(1)
        end do
      end if
      if(td%electric_enhancement_calculate_omega) then
        !message(1) = "<><><><><><> PlasmOctopus td.F90 ------ total_potential_effective - done"
        !call messages_info(1)
        do idim = 1, gr%mesh%sb%dim
          do iomega = 1, td%electric_enhancement_nomega
            do ispin = 1, st%d%nspin
              electric_enhancement_FFT(:,idim,iomega,ispin) = &
                electric_enhancement_FFT(:,idim,iomega,ispin) + &
                td%dt * electric_field_effective(:,idim,ispin) * &
                sin(td%electric_enhancement_omega(iomega)*iter*td%dt) * &
                exp(-td%electric_enhancement_sigma*iter*td%dt)
            end do
          end do
        end do
      end if
      if(td%electric_enhancement_calculate_td) then
        do iii=1,td%electric_enhancement_ntd
          electric_enhancement_td_found = .false.
          if(td%electric_enhancement_td(iii)==iter) then
            electric_enhancement_td_found = .true.
            exit
          end if
        end do
        if(electric_enhancement_td_found) then
          do idim = 1, gr%mesh%sb%dim
            do ispin=1,st%d%nspin
              electric_field_effective_td(:, idim, iii, ispin) = electric_field_effective(:, idim, ispin)
            end do
          end do
        end if
      end if
      !message(1) = "<><><><><><> PlasmOctopus td.F90 ------ electric_enhancement_calculate - done"
      !call messages_info(1)
      ! Fourier transform of the density: calculation of plasmonic modes
      ! Preparing the integrand of the Fourier transform according to Eq. 11 in the paper: M. Kuisma et al, PRB 91, 115431 (2015).
      ! NEW: Using the simpler implementation in Zhang et al., PRB 90, 161407(R) (2014)
      if(td%plasmonics_calculate) then
        do iomega = 1, td%plasmonics_nomega
          do ispin = 1, st%d%nspin
            density_FFT(:,iomega,ispin) = density_FFT(:,iomega,ispin) + td%dt*(st%rho(:,ispin)-rho_0(:,ispin)) &
              * exp(-(iter*td%dt)*td%plasmonics_sigma) &
              * sin(td%plasmonics_omega(iomega)*iter*td%dt)
            !do ip = 1, gr%mesh%np_part
            !  !PRB 90, 161407(R) (2014)
            !  density_FFT(ip,iomega,ispin) = density_FFT(ip,iomega,ispin) + td%dt*(st%rho(ip,ispin)-rho_0(ip,ispin)) &
            !  * exp(-(iter*td%dt)*td%plasmonics_sigma) &
            !  * sin(td%plasmonics_omega(iomega)*iter*td%dt)
            !end do
            !write(message(1), '(a,i4.4,a,i4.4)') 'PlasmOctopus >>>> plasmonics_nomega: ',ispin, ' ', iomega
            !call messages_info(1)
          end do
         
          !write(*,*) 'Done with density_FFT'
          if(td%plasmonics_output_wfmod) then
            !Contribution of the orbitals to the plasmonic mode
            do inik = st%d%kpt%start,st%d%kpt%end
              do inst = st%st_start,st%st_end
                call states_elec_get_state(st, sys%hm%psolver%der%mesh, inst, inik, zpsi_temp(:, :, inst, inik))
  
                do ispin = 1, st%d%nspin
                  !do ip = 1, gr%mesh%np
                  !  if(states_are_complex(st)) then
                  !    sqr_wf = (real(wfmod_0(ip, ispin, inst, inik), REAL_PRECISION)*real(wfmod_0(ip, ispin, inst, inik), REAL_PRECISION) + aimag(wfmod_0(ip, ispin, inst, inik))*aimag(wfmod_0(ip, ispin, inst, inik)))
                  !    sqr_wf2 = (real(zpsi_temp(ip, ispin, inst, inik), REAL_PRECISION)*real(zpsi_temp(ip, ispin, inst, inik), REAL_PRECISION) + aimag(zpsi_temp(ip, ispin, inst, inik))*aimag(zpsi_temp(ip, ispin, inst, inik)))
                  !  else
                  !    sqr_wf = wfmod_0(ip, ispin, inst, inik)*wfmod_0(ip, ispin, inst, inik)
                  !    sqr_wf2 = zpsi_temp(ip, ispin, inst, inik)*zpsi_temp(ip, ispin, inst, inik)
                  !  end if
                  !  wfmod_FFT(inik,inst,ip,iomega,ispin) = wfmod_FFT(inik,inst,ip,iomega,ispin) + (sqr_wf2 - sqr_wf) &
                  !  * exp(-(iter*td%dt)*td%plasmonics_sigma) &
                  !  * sin(td%plasmonics_omega(iomega)*iter*td%dt)
                  !end do
                  if(states_are_complex(st)) then
                    !sqr_wf = (real(wfmod_0(:, ispin, inst, inik), REAL_PRECISION)*real(wfmod_0(:, ispin, inst, inik), REAL_PRECISION) + aimag(wfmod_0(:, ispin, inst, inik))*aimag(wfmod_0(:, ispin, inst, inik)))
                    !sqr_wf2 = (real(zpsi_temp(:, ispin, inst, inik), REAL_PRECISION)*real(zpsi_temp(:, ispin, inst, inik), REAL_PRECISION) + aimag(zpsi_temp(:, ispin, inst, inik))*aimag(zpsi_temp(:, ispin, inst, inik)))

                    wfmod_FFT(inik,inst,:,iomega,ispin) = wfmod_FFT(inik,inst,:,iomega,ispin) + &
                    ((real(zpsi_temp(:, ispin, inst, inik), REAL_PRECISION)*real(zpsi_temp(:, ispin, inst, inik), REAL_PRECISION) + aimag(zpsi_temp(:, ispin, inst, inik))*aimag(zpsi_temp(:, ispin, inst, inik))) - &
                    (real(wfmod_0(:, ispin, inst, inik), REAL_PRECISION)*real(wfmod_0(:, ispin, inst, inik), REAL_PRECISION) + aimag(wfmod_0(:, ispin, inst, inik))*aimag(wfmod_0(:, ispin, inst, inik)))) &
                    * exp(-(iter*td%dt)*td%plasmonics_sigma) &
                    * sin(td%plasmonics_omega(iomega)*iter*td%dt)
                  else
                    !sqr_wf = wfmod_0(:, ispin, inst, inik)*wfmod_0(:, ispin, inst, inik)
                    !sqr_wf2 = zpsi_temp(:, ispin, inst, inik)*zpsi_temp(:, ispin, inst, inik)

                    wfmod_FFT(inik,inst,:,iomega,ispin) = wfmod_FFT(inik,inst,:,iomega,ispin) + &
                    (zpsi_temp(:, ispin, inst, inik)*zpsi_temp(:, ispin, inst, inik) - &
                    wfmod_0(:, ispin, inst, inik)*wfmod_0(:, ispin, inst, inik)) &
                    * exp(-(iter*td%dt)*td%plasmonics_sigma) &
                    * sin(td%plasmonics_omega(iomega)*iter*td%dt)
                  end if
                  !write(message(1), '(a,i4.4,a,i4.4,a,i4.4)') 'PlasmOctopus >>>> plasmonics_output_wfmod: ',inik, ' ',inst, ' ', ispin
                  !call messages_info(1)
                end do
      ...
      !message(1) = "<><><><><><> PlasmOctopus td.F90 ------ plasmonics_calculate - done"
      !call messages_info(1)
    ...
    ! 24-08-2015
    ! Plasmonics
    ! Write the plasmonic modes
    ! Write in XSF format
    ! Based on: SAFE_ALLOCATE(density_FFT(1:gr%fine%mesh%np_part, 1:td%plasmonics_nomega, 1:sys%st%d%nspin))
    write(*,*) '<><><><><><> PlasmOctopus: mode energies: ',td%plasmonics_nomega, sys%st%d%nspin
    if(td%plasmonics_calculate) then
      do iomega = 1, td%plasmonics_nomega
        do ispin=1, sys%st%d%nspin
          !write(*,*) '<><><><><><> PlasmOctopus Writing mode:',iomega,ispin
          write(plasmonics_filename, '(a,i1,a,i4.4)') 'plasmonic_mode_',ispin,'_',iomega
          write(*,*) '<><><><><><> PlasmOctopus Created file'
          call dio_function_output(OPTION__OUTPUTFORMAT__XCRYSDEN, "td.general", &
              plasmonics_filename, sys%namespace, gr%mesh, &
              density_FFT(:,iomega,ispin),&
              unit_one, ierr, sys%geo)
          ...
          if(td%plasmonics_output_wfmod) then
            do inik = st%d%kpt%start,st%d%kpt%end
              do inst = st%st_start,st%st_end
                write(plasmonics_filename, '(a,i4.4,a,i4.4,a,i1,a,i4.4)') 'plasmonic_wf_mode_',inst,'_',inik,'_',ispin,'_',iomega
                call dio_function_output(OPTION__OUTPUTFORMAT__XCRYSDEN, "td.general", &
                    plasmonics_filename, sys%namespace, gr%mesh, &
                    wfmod_FFT(inik,inst,:,iomega,ispin),&
                    unit_one, ierr, sys%geo)
                
              end do
            end do
          end if
          write(*,*) '<><><><><><> PlasmOctopus Finished plasmonics_output_wfmod'
          !total_potential_effective = M_ZERO
         
          !call dpoisson_solve(sys%hm%psolver, total_potential_effective, density_FFT(:,iomega,ispin), all_nodes = .false.)
          !message(1) = "<><><><><><> PlasmOctopus td.F90 ------ poisson - done"
          !call messages_info(1)
          
          !call dderivatives_grad(gr%der, total_potential_effective(:), electric_field_effective(:, :, ispin))
          !message(1) = "<><><><><><> PlasmOctopus td.F90 ------ dderivatives_grad - done"
          !call messages_info(1)


          
          !write(electric_enhancement_filename, '(a,i1,a,i4.4)') 'electric_enhancement_mode_',ispin,'_',iomega
          !call dio_function_output(OPTION__OUTPUTFORMAT__XCRYSDEN, "td.general", &
          !    electric_enhancement_filename, sys%namespace, sys%hm%psolver%der%mesh, &
          !    total_potential_effective(:),&
              !(electric_field_effective(:,1,ispin)**2 &
              !+electric_field_effective(:,2,ispin)**2 &
              !+electric_field_effective(:,3,ispin)**2),&
          !    unit_one, ierr, sys%geo)
          write(*,*) '<><><><><><> PlasmOctopus Finished first loop'
        end do
      end do
    end if
    
    ! Electric field enhancement
    ! Write the electric field enhancement modes
    ! Write in XSF format
    if(td%electric_enhancement_calculate_omega) then
      do iomega = 1, td%electric_enhancement_nomega
        do ispin=1,st%d%dim
          write(electric_enhancement_filename, '(a,i4.4)') 'electric_enhancement_mode_',iomega
          !file_electric_enhancement_mode = io_open(electric_enhancement_filename, action='write')
          call dio_function_output(OPTION__OUTPUTFORMAT__XCRYSDEN, "td.general", &
            electric_enhancement_filename, sys%namespace, gr%mesh, &
            (electric_enhancement_FFT(:,1,iomega,ispin)**2 &
            +electric_enhancement_FFT(:,2,iomega,ispin)**2 &
            +electric_enhancement_FFT(:,3,iomega,ispin)**2),&
            unit_one, ierr, sys%geo)
        end do
      end do
    end if
    if(td%electric_enhancement_calculate_td) then
      do iii = 1, td%electric_enhancement_ntd
        do idim = 1, gr%mesh%sb%dim
          do ispin=1,st%d%nspin
            write(electric_enhancement_td_filename, '(a,i1,a,i1,a,i10.10)') 'electric_enhancement_td_',idim,'_',ispin,'_',td%electric_enhancement_td(iii)
            call dio_function_output(OPTION__OUTPUTFORMAT__XCRYSDEN, "td.general", &
                electric_enhancement_td_filename, sys%namespace,gr%mesh, &
                electric_field_effective_td(:,idim,iii,ispin),&
                unit_one, ierr, sys%geo)
          end do
        end do
      end do
    ...
    SAFE_DEALLOCATE_A(electric_field_effective)
    SAFE_DEALLOCATE_A(electric_field_effective_td)
    SAFE_DEALLOCATE_A(wfmod_FFT)
    SAFE_DEALLOCATE_A(density_FFT)
    SAFE_DEALLOCATE_A(total_potential_effective)
    SAFE_DEALLOCATE_A(zpsi_temp)
    !PlasmOctopus end
